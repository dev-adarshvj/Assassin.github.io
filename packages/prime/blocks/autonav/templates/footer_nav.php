...
if (count($navItems) > 0) {
    if($key % 2 == 0)
     {
       echo '<ul class="nav footer_main_nav">'; //opens the top-level menu
     }


    foreach ($navItems as $ni) {
         if($ni->cObj->getAttributeValue('footer_nav'))
      {
        echo '<li class="' . $ni->classes . '">'; //opens a nav item
        echo '<a href="' . $ni->url . '" target="' . $ni->target . '" class="' . $ni->classes . '" title="' .h($ni->name). '">' . h($ni->name) . '</a>';
    }
        if ($ni->hasSubmenu) {
    ...
    // if($key % 2 == 3 || $key == count($navItems)-1)
    //         {
    //           echo '</ul>';
    //         }
