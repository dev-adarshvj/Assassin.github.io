<?php
namespace Concrete\Package\ProfessionalManagement\Src\MailExtended;
use Config;
...
class MailService
{
	...
	public function addAttachments($files) {
		$i=0;
			foreach($files as $file) {
				...
				$mp = new MimePart(file_get_contents($file['path']));
				$mp->type = $file['mime'];
				$mp->disposition = Mime::DISPOSITION_ATTACHMENT;
				$mp->encoding = Mime::ENCODING_BASE64;
				$mp->filename = $file['name'];
			// Add mimepart to attachments.
				$this->attachments[] = $mp;
					$i++;
					//print_r($this);
     ...
     * @todo documentation
     */
    public function generateEmailStrings($arr)
    {
        $str = '';
